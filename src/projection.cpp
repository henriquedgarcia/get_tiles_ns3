...
Projection::Projection(const Resolution& resolution, const Tiling& tiling) : resolution(resolution), tiling(tiling) {
...
vector<Tile> Projection::make_tile_list() {
  ...
  for (int h = 0; h < this->tiling[1]; h++) {
    for (int w = 0; w < this->tiling[0]; w++) {
      PointMN position(w * (this->tile_resolution[0]), h * (this->tile_resolution[1]));
